...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const keyNames = document.querySelectorAll('.piano button[data-key]');

  document.addEventListener('keydown', function (event) {
      // Check if the pressed key is "N"
      if (event.key.toUpperCase() === 'N') {
          // Toggle the visibility of key names
          keyNames.forEach(keyName => {
              keyName.classList.toggle('hidden');
          });
      }

      // Check if the pressed key is in the data-key attribute of the piano keys
      const pressedKey = event.key.toUpperCase();
      const pianoKey = document.querySelector(`.piano button[data-key="${pressedKey}"]`);

      // If a piano key is found, simulate a click event to play the corresponding note
      if (pianoKey) {
          pianoKey.click();
      }
  });
